...
nba_seasons_offense <- select(nba_seasons, Year, Tm, Player, Offense=Offense_1)
...
offense <- nba_seasons_offense %>%
              filter(Year >= 2000) %>%
              group_by(Year, Tm) %>%
              summarize(mean_offense=sum(Offense))
...
for (row in 1:nrow(nba_seasons)) {
 ...
 if (nba_seasons[row, "Tm"] == "LAL") {
  ...
  if (nba_seasons[row, "Tm"] == "DAL") {
   offense[row, "Tm"] <- "Dallas Mavericks"
  }
