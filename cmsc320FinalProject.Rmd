...
These graphs show a team's average player efficience rating (PER) on the Y-axis and compares it to the team's average payroll on the X-axis
...
nba_data <- left_join(moneyball, offense_champs, by=c("Year"="Year"))
nba_data
...
for (t in times) {
  x <- filter(nba_data, time_period == t)
  gg1 <- ggplot(x, aes(x=payroll, y=win_p)) +
        geom_point(aes(color= is_champ)) +
        labs(x="Payroll", y="Winning Percentage", fill= "Champtions") + 
        geom_smooth(method=lm) #add regression line
        gg1 + facet_wrap( ~ cut(Year, breaks =5), ncol=2)
